...
import { CollegeData, Regulation, Program } from "@/app/@types/college";
...
export default function CollegeDataForm({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.programs.length > 0) {
        const program = initialData.programs[0];
        setSelectedProgram(program.name);
        setSelectedSpecializations(program.specializations);
        setSelectedYears(program.years);
        setSelectedRegulations(program.regulations);
      }
    }
  }, [initialData]);
  ...
  const handleDialogOpen = () => {
    if (!isEditMode) {
      // Reset form state for new entries
      setNewData({
        collegeName: "",
        regulatoryBody: "",
        domain: "",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end space-x-2 pt-4">
        <Button variant="outline" onClick={() => onSubmit(newData)}>
          Cancel
        </Button>
        <Button onClick={() => onSubmit(newData)}>Save</Button>
      </div>
    </div>
  );
}
